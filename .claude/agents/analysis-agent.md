...
2. `analysis/<domain>/<title>/PROPOSAL.md` を作成する
   ...
   - 動機・背景
   - 分析タイプの明示（EDA のみ / CDA を含む）
   - 事前登録（EDA: 見る指標・期間・対象の列挙 / CDA: 仮説・有意水準）
   - スコープ（対象・対象外）
   - **データ取得計画**（以下を各データソースごとに記載）:
     - データ名・提供機関・URL
     - ライセンス（CC BY / パブリックドメイン / 要許可 など）
     - 取得方法（API / 手動ダウンロード）
     - API の場合: 公式ドキュメント URL・認証方式・登録の要否・レート制限
     - 利用上の注意点（再配布制限・引用必須・商用利用可否 など）
