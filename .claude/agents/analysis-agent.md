...
### 初回呼び出し（`review_feedback` が null の場合）

#### Phase 1 — 着手前チェック（DoD Sec 1）

1. `analysis/<title>/` ディレクトリ構造を作成する（DoD Sec 7 のテンプレート通り）
2. `analysis/<title>/PROPOSAL.md` を作成する
...
### 差し戻し呼び出し（`review_feedback` が非 null の場合）
